...
class RegexMatcherPipe:
    ...
    def __init__(self, nlp, name, regex: Mapping[str, str], stopwords: List[str] = None,
                 ...
                 on_match: Callable[[Doc, Span], Doc] = None):
        '''
            ...
            Input:
                ...
                - on_match: custom on match callback
        '''
        self.nlp = nlp
        self.name = name
    ...
    def set_on_match(self, on_match: Callable[[Doc, Span], Doc]):
    ...
    def __call__(self, doc):
            ...
            for match in self.patterns[k].finditer(doc.text, concurrent=True):
                ...
                if span and len(span.text) >= len(match.group()):
                    ...
                    if self.on_match is not None:
                        # run custom callback
                        self.on_match(doc, span, entities)
                    else:
        ...
        if self.on_match is None and new_entities:
